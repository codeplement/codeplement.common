...
import { JwtHelperService, GenericSubjects } from '@root/services';
...
export class RequestInterceptor implements HttpInterceptor {
    ...
    isUnCachableRoutes(request: HttpRequest<any>): boolean {
        ...
        return (
            this.unCachableRoutes.findIndex(
                route =>
                    typeof route === 'string'
                        ? route === url
    ...
    async handleInterception(
        ...
        if (value) {
            return next.handle(request)
                .pipe(tap((event) => this.handleResponse(request, event), error => this.handleResponse(request, error)))
                .toPromise();
        }
        else {
    ...
    async handleGetRequest(request: HttpRequest<any>) {
        const requestUrl = parse(request.url, false, true);
        const cache = await this.cacheStorage.open(stringWithVersion(this.config.version, this.location.path(true)));
    ...
    async handleResponse(request: HttpRequest<any>, httpEvent: HttpEvent<any>) {
        const cacheName = stringWithVersion(this.config.version, this.location.path(true));
        if (httpEvent instanceof HttpResponse) {
            ...
            if (httpEvent instanceof HttpErrorResponse) {
                if (httpEvent.status === 401) {
                    this.genericSubjects.get<boolean>('authenticationStatus$').next(false);
                }
            }
        }
    }
    intercept(
        ...
        if (
            this.isUnCachableRoutes(request)
        ) {
            return next.handle(request);
        }
